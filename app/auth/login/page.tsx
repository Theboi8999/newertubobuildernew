    ...
    <div className="min-h-screen bg-brand-bg flex items-center justify-center px-4">
      <div className="w-full max-w-md card p-8">
        ...
        <div className="space-y-3">
          ...
          <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="Password" onKeyDown={e => e.key === 'Enter' && handleSubmit()} className="input" />
        </div>
        {error && <p className="text-red-400 text-sm mt-3 p-3 bg-red-400/10 rounded-lg">{error}</p>}
        {success && <p className="text-green-400 text-sm mt-3 p-3 bg-green-400/10 rounded-lg">{success}</p>}
