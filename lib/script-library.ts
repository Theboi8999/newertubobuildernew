...
export async function detectMissingScripts(prompt: string): Promise<string[]> {
  try {
    const response = await anthropic.messages.create({
      ...
      max_tokens: 400,
      system: `You identify scripting concepts needed for a Roblox generation request that are NOT in this list of already-known concepts: ${KNOWN_MODULES.join(', ')}.
...
export async function generateNewScript(concept: string): Promise<StoredScript | null> {
  try {
    const response = await anthropic.messages.create({
      model: 'claude-sonnet-4-20250514',
      max_tokens: 3000,
      system: `You are a veteran Roblox Luau scripter. Generate a complete, reusable script pattern for the requested concept.
The script must be:
...
Output ONLY JSON:
{
  ...
  "quality_score": 85
      ...
      messages: [{
        role: 'user',
        content: `Generate a complete Roblox Luau script pattern for: "${concept}"`
      }]
    })
