...
export function watermarkRbxmx(rbxmx: string, generationId: string, userId: string): string {
  ...
  const watermarkItem = `  <Item class="StringValue" referent="WATERMARK_001">
    <Properties>
      <string name="Name">TurboBuilder_ID</string>
      <string name="Value">${generationId}|${userId}|${timestamp}</string>
    </Properties>
  </Item>`
  // Insert as first child inside ROOT_MODEL — after the Model Properties block
  return rbxmx.replace(
    /<Item class="Model" referent="ROOT_MODEL">\s*<Properties>[\s\S]*?<\/Properties>/,
    (match) => match + '\n' + watermarkItem
  )
}
