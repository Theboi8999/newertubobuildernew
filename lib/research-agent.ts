...
async function wiki(name:string):Promise<string> { try { const r=await fetch(`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(name.replace(/\s+/g,'_'))}`,{signal:AbortSignal.timeout(5000)}); if(!r.ok)return ''; const d=await r.json(); return String(d.extract||'').substring(0,400) } catch{return ''} }
...
async function knowledge():Promise<string> { try { const sb=createAdminClient(); const {data}=await sb.from('ai_knowledge').select('content').eq('is_active',true).eq('system_type','builder').limit(2); return data?.map((k:any)=>k.content).join('\n').substring(0,400)||'' } catch{return ''} }
