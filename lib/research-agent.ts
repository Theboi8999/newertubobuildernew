...
async function knowledge():Promise<string> { try { const sb=createAdminClient(); const {data,error}=await sb.from('ai_knowledge').select('content').eq('is_active',true).eq('system_type','builder').limit(2); if(error){console.error('[research] knowledge query error:',error.message);return ''} return data?.map((k:any)=>k.content).join('\n').substring(0,400)||'' } catch(e){console.error('[research] knowledge fetch error:',e);return ''} }
